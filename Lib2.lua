...
local Library = { } do
    ...
    Library.ESPPreview = function(Self, Params)
        ...
        local function UpdateBounds()
            ...
            local Scale = (Camera.ViewportSize.Y * 1.5)
                / (Screen.Z * 2 * math.tan(math.rad(Camera.FieldOfView) / 2))
            local Width = 2.0833 * Scale
            local Height = 3.6364 * Scale

            Items.RenderFrame.Instance.Position = UDim2.fromOffset(
                math.floor(Screen.X - Width / 2),
                math.floor(Screen.Y - Height / 2.5)
            )
            Items.RenderFrame.Instance.Size = UDim2.fromOffset(math.ceil(Width), math.ceil(Height))
        end
