if getgenv().Havoc and getgenv().Havoc.Unload then
    getgenv().Havoc:Unload()
end
...
local Library = { } do
    ...
    Library.Directory = "Havoc"
    Library.ConfigFolder = "Havoc/Configs"
    Library.AssetsFolder = "Havoc/Assets"
    ...
    local function CustomImage(Source, Name)
        if type(Source) ~= "string" or Source == "" then return "" end
        if ImageCache[Source] then return ImageCache[Source] end
        if Source:match("^rbxasset://") and not Source:match("^rbxasset://%d+$") then return Source end

        local Ok, Asset = pcall(function()
            local Id = Source:match("^rbxassetid://(%d+)$") or Source:match("^rbxasset://(%d+)$") or Source:match("^(%d+)$")
            local Url = Source
            local Key = Name
            ...
            Key = tostring(Key or HttpService:GenerateGUID(false)):gsub("[^%w_%-]", "_")
            local Path = Library.AssetsFolder .. "/" .. Key .. ".png"
        ...
        Asset = Ok and Asset or ""
        ImageCache[Source] = Asset
        return Asset
    end

    Library.Logo = CustomImage(
        "https://raw.githubusercontent.com/skinnedforcash/Havochax/main/w.png",
        "HavocLogo"
    )

    local function AvatarImage(UserId, Resolution)
        local Key = "Avatar_" .. UserId .. "_" .. Resolution
        local Path = Library.AssetsFolder .. "/" .. Key .. ".png"
    ...
    local function ResolveIcon(Icon)
        if type(Icon) == "number" then
            return CustomImage(tostring(Icon), "Icon_" .. Icon)
        end
        ...
        if string.match(Icon, "^rbxassetid://") or string.match(Icon, "^rbxasset://") then
            return CustomImage(Icon)
        end

        if string.match(Icon, "^%d+$") then
            return CustomImage(Icon, "Icon_" .. Icon)
        end
        ...
        if IconPack then
            ...
            if Ok and Result then
                if type(Result) == "table" then
                    local Sheet = Result[1]
                    local Data = Result[2] or {}

                    return CustomImage(Sheet), Data.ImageRectPosition, Data.ImageRectSize
                end

                if Result ~= "rbxassetid://0" then
                    return CustomImage(Result)
                end
            end
        end
    ...
    Library.Window = function(Self, Params)
        ...
        local Window = {
            Name = Params.Name or "HAVOC",
            Icon = Params.Icon and Params.Icon ~= "" and Params.Icon or Library.Logo,
            IsOpen = true,
            Tabs = { },
            Current = nil,
            ContentW = W - 30,
        ...
        local function MakeAvatar(Parent, Props)
            local Avatar = Library:Create("ImageLabel", {
                ...
                ZIndex = Props.Z,
                BorderSizePixel = 0,
                Image = AvatarImage(LocalPlayer.UserId, Props.Res)
    ...
    Library.ESPPreview = function(Self, Params)
        ...
        local Window = Self.Window or (Self.SubTab and Self.SubTab.Window) or Self
        local Flags = Library.Flags
        local Preview = {
            Items = { },
            Health = Params.Health or 73,
            MaxHealth = Params.MaxHealth or 100,
            Name = Params.Name or LocalPlayer.Name,
            Distance = Params.Distance or 128,
            Weapon = Params.Weapon or "M4A1",
            AnimateHealth = Params.AnimateHealth ~= false,
        ...
        if not Library.ESPFonts then
            local function ImportFont(Name, Url)
                local Ok, Result = pcall(function()
                    if not isfolder("Havoc") then makefolder("Havoc") end
                    if not isfolder("Havoc/Fonts") then makefolder("Havoc/Fonts") end

                    local FilePath = string.format("Havoc/Fonts/%s.ttf", Name)
                    local FacePath = string.format("Havoc/Fonts/%s.font", Name)

                    if not isfile(FilePath) then
                        writefile(FilePath, game:HttpGet(Url))
                    end

                    writefile(FacePath, HttpService:JSONEncode({
                        name = Name,
                        faces = {
                            {
                                name = "Regular",
                    ...
                    return Font.new(getcustomasset(FacePath), Enum.FontWeight.Regular, Enum.FontStyle.Normal)
                end)

                return Ok and Result or Font.fromEnum(Enum.Font.Code)
            end

            Library.ESPFonts = {
                Main = ImportFont("ProggyClean", "https://github.com/i77lhm/storage/raw/refs/heads/main/fonts/ProggyClean.ttf"),
                Bold = ImportFont("TahomaBold", "https://github.com/i77lhm/storage/raw/refs/heads/main/fonts/tahoma_bold.ttf")
            }
        end

        local function Create(Class, Properties)
            Properties.Name = "\0"
            return Library:Create(Class, Properties)
        end

        local function Color(Flag, Default)
            ...
            if type(Value) == "table" then
                local Parsed = Value.Color or Value[1]

                if not Parsed and type(Value.__color) == "string" then
                    local Ok, Result = pcall(Color3.fromHex, Value.__color)
                    Parsed = Ok and Result or nil
                end

                return Parsed or Default, Value.__alpha or Value.Alpha or Value.Transparency or Value[2] or 0
            end
        ...
        Items.Title = MakeText({
            Parent = Items.Topbar.Instance,
            Text = Params.Title or "ESP Preview",
            TextSize = 15,
            Bold = true,
            Anchor = Vector2.new(0, 0.5),
            Pos = UDim2.new(0, 14, 0.5, 0),
        ...
        Items.Close = MakeButton({
            Parent = Items.Topbar.Instance,
            Pos = UDim2.new(1, -34, 0.5, -10),
            Size = UDim2.fromOffset(20, 20),
            Z = 33
        })
        ...
        Items.Viewport = Create("ViewportFrame", {
            Parent = Items.Root.Instance,
            BackgroundTransparency = 1,
        ...
        Items.Camera = Create("Camera", {
            ...
            CFrame = CFrame.lookAt(Vector3.new(0, 2.5, 6), Vector3.new(0, 0.65, 0))
        })
        Items.Viewport.Instance.CurrentCamera = Items.Camera.Instance

        Items.RenderFrame = Create("Frame", {
            Parent = Items.Viewport.Instance,
            Position = UDim2.new(0.22, 0, 0.08, 0),
            Size = UDim2.new(0.56, 0, 0.84, 0),
            BackgroundColor3 = Color3.new(1, 1, 1),
            BackgroundTransparency = 1,
            BorderSizePixel = 0,
            ZIndex = 34
        ...
        Items.FillGradient = Create("UIGradient", {
        ...
        Items.BoxAccent = Create("UIStroke", {
            Parent = Items.RenderFrame.Instance,
            Thickness = 1,
            Color = Color3.new(1, 1, 1),
        ...
        Items.Glow = Create("ImageLabel", {
            Parent = Items.RenderFrame.Instance,
            Visible = false,
            Image = CustomImage("rbxassetid://18245826428", "ESP_Glow"),
        ...
        Items.GlowGradient = Create("UIGradient", {
        ...
        Items.HealthText = Create("TextLabel", {
            Parent = Items.Left.Instance,
            LayoutOrder = -2,
            Text = tostring(Preview.Health),
            FontFace = Library.ESPFonts.Main,
            TextSize = 12,
        ...
        local function BottomText(Text)
        ...
        Items.Distance = BottomText(tostring(Preview.Distance) .. "M")
        Items.Weapon = BottomText(string.upper(Preview.Weapon))

        Items.Highlight = Create("Highlight", {
            Parent = Items.World.Instance,
        ...
        if Character then
            ...
            if Ok and Model then
                ...
                Model.Parent = Items.World.Instance
                Items.Highlight.Instance.Adornee = Model
        ...
        local function UpdateBounds()
            ...
            Camera.CFrame = CFrame.lookAt(Target + Vector3.new(0, 0.25, 6), Target)
            ...
            Items.RenderFrame.Instance.Position = UDim2.fromOffset(
        ...
        function Preview:Refresh()
            ...
            local Glow = Box and Flags["ESP Box Glow"] and true or false
            local Healthbar = Flags["ESP Health Bar"] and true or false
            local HealthText = Healthbar and Flags["ESP Health Text"] and true or false
            local NameTag = Flags["ESP Name Tag"] and true or false
            local DistanceTag = Flags["ESP Distance Tag"] and true or false
            local WeaponTag = Flags["ESP Held Item"] and true or false
            local BoxHigh = Color("ESP Box High Color", Color3.new(1, 1, 1))
            local BoxLow = Color("ESP Box Low Color", Color3.new(1, 1, 1))
            local FillHigh, FillHighAlpha = Color("ESP Fill High Color", Color3.new(1, 1, 1))
            local FillLow, FillLowAlpha = Color("ESP Fill Low Color", Color3.new(1, 1, 1))
            local GlowHigh, GlowHighAlpha = Color("ESP Box Glow High Color", Color3.new(1, 1, 1))
            local GlowLow, GlowLowAlpha = Color("ESP Box Glow Low Color", Color3.new(1, 1, 1))
            local HealthHigh = Color("ESP Health Bar High Color", Color3.fromRGB(0, 255, 0))
            local HealthMedium = Color("ESP Health Bar Medium Color", Color3.fromRGB(255, 165, 0))
            local HealthLow = Color("ESP Health Bar Low Color", Color3.fromRGB(255, 0, 0))
            local NameColor, NameAlpha = Color("ESP Name Tag Color", Color3.new(1, 1, 1))
            local DistanceColor, DistanceAlpha = Color("ESP Distance Tag Color", Color3.new(1, 1, 1))
            local WeaponColor, WeaponAlpha = Color("ESP Held Item Color", Color3.new(1, 1, 1))
            local Ratio = math.clamp(Preview.Health / math.max(Preview.MaxHealth, 1), 0, 1)

            Items.BoxOutline.Instance.Enabled = false
            Items.BoxAccent.Instance.Enabled = Box
            Items.BoxInnerStroke.Instance.Enabled = false
            Items.BoxAccent.Instance.Color = BoxHigh:Lerp(BoxLow, math.clamp(Preview.Distance / math.max(tonumber(Flags["ESP Max Distance"]) or 1000, 1), 0, 1))
            Items.RenderFrame.Instance.BackgroundTransparency = Fill and 0 or 1
            Items.FillGradient.Instance.Color = ColorSequence.new(FillHigh, FillLow)
            local FillTransparency = 1 - math.clamp(tonumber(Flags["ESP Box Fill Opacity"]) or 0.35, 0, 1)
            Items.FillGradient.Instance.Transparency = NumberSequence.new(FillTransparency)
            Items.Glow.Instance.Visible = Glow
            Items.GlowGradient.Instance.Color = ColorSequence.new(GlowHigh, GlowLow)
            Items.GlowGradient.Instance.Transparency = NumberSequence.new({
                NumberSequenceKeypoint.new(0, GlowHighAlpha),
                NumberSequenceKeypoint.new(1, GlowLowAlpha)
            })
            Items.Healthbar.Instance.Visible = Healthbar
            Items.HealthText.Instance.Visible = HealthText
            Items.HealthbarGradient.Instance.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, HealthHigh),
                ColorSequenceKeypoint.new(0.5, HealthMedium),
                ColorSequenceKeypoint.new(1, HealthLow)
            })
            Items.HealthbarFade.Instance.Size = UDim2.new(1, -2, 1 - Ratio, 0)
            Items.HealthText.Instance.Text = tostring(math.ceil(Preview.Health))
            Items.HealthText.Instance.TextColor3 = HealthLow:Lerp(HealthMedium, Ratio):Lerp(HealthHigh, Ratio)
            Items.HealthText.Instance.Size = UDim2.new(0, 0, 1 - Ratio, Items.HealthText.Instance.TextBounds.Y - 3)
            Items.Name.Instance.Visible = NameTag
            Items.Name.Instance.Text = Preview.Name
            Items.Name.Instance.TextColor3 = NameColor
            Items.Name.Instance.TextTransparency = NameAlpha
            Items.Bottom.Instance.Visible = DistanceTag or WeaponTag
            Items.Distance.Instance.Visible = DistanceTag
            Items.Distance.Instance.Text = math.ceil(Preview.Distance) .. "M"
            Items.Distance.Instance.TextColor3 = DistanceColor
            Items.Distance.Instance.TextTransparency = DistanceAlpha
            Items.Weapon.Instance.Visible = WeaponTag
            Items.Weapon.Instance.Text = string.upper(Preview.Weapon)
            Items.Weapon.Instance.TextColor3 = WeaponColor
            Items.Weapon.Instance.TextTransparency = WeaponAlpha

            local Chams = Flags["ESP Chams"] and Preview.Player ~= nil
            local InnerColor, InnerAlpha = Color("ESP Chams Inner Color", Color3.new(1, 1, 1))
            local OuterColor, OuterAlpha = Color("ESP Chams Outer Color", Color3.new(1, 1, 1))

            Items.Highlight.Instance.Enabled = Chams
            Items.Highlight.Instance.FillColor = InnerColor
            Items.Highlight.Instance.FillTransparency = InnerAlpha
            Items.Highlight.Instance.OutlineColor = OuterColor
            Items.Highlight.Instance.OutlineTransparency = OuterAlpha
        end
        ...
        function Preview:SetHealth(Health, MaxHealth)
            Preview.Health = tonumber(Health) or Preview.Health
            Preview.MaxHealth = tonumber(MaxHealth) or Preview.MaxHealth
            Preview:Refresh()
        end

        function Preview:SetSubject(Name, Distance, Weapon)
            Preview.Name = Name or Preview.Name
            Preview.Distance = tonumber(Distance) or Preview.Distance
            Preview.Weapon = Weapon or Preview.Weapon
            Preview:Refresh()
        end
        ...
        Library:Connect(RunService.RenderStepped, function(Delta)
            ...
            if Active then
                if Preview.AnimateHealth then
                    HealthTime = HealthTime + Delta
                    Preview.Health = Preview.MaxHealth * (0.505 + math.sin(HealthTime * 0.8) * 0.495)
                end
        ...
        Library:Connect(Items.Viewport.Instance.InputBegan, function(Input)
        ...
        Library:Connect(Items.Viewport.Instance.InputEnded, function(Input)
        ...
        Library:Connect(Items.Viewport.Instance.InputChanged, function(Input)
            if not Rotating or not LastPosition or not Preview.Player then return end
            if Input.UserInputType ~= Enum.UserInputType.MouseMovement and Input.UserInputType ~= Enum.UserInputType.Touch then return end

            local Delta = Input.Position - LastPosition
            local Pivot = Preview.Player:GetPivot()
            local Position = Pivot.Position
            Preview.Player:PivotTo(CFrame.new(Position) * (Pivot - Position) * CFrame.Angles(0, math.rad(-Delta.X * 0.7), 0))
            LastPosition = Input.Position
        end)
    ...
    Library.Watermark = function(Self, Params)
        ...
        local Icon = Params.Icon and Params.Icon ~= "" and Params.Icon
            or (Self and Self.Icon and Self.Icon ~= "" and Self.Icon)
            or Library.Logo
        local Items = { }
        local Order = 0
